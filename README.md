...
- **`concept` が一致する語は、公開中の全エンジン分をまとめて表示する**（1 対 1 ではなく 1 対多）。バッジは各語の `match` を個別に表示するので、同じ概念でもエンジンごとに `"同義"` / `"近い"` が混在してよい
...
- `"同義"` は緑バッジ、`"近い"` は黄バッジ＋「完全に同じではない」の注記で見た目を分ける（例: UE `Roughness` と Unity `Smoothness` は値の向きが逆なので `"近い"`）
